...
async function Page({params}) {
    ...
    return (
        <>{data.success !== false ? (
            <>
                <div className='relative aspect-video lg:aspect-[2.66/1] overflow-hidden rounded-[1.25rem]'>
                    <Image
                        src={data.backdrop_path ? `https://image.tmdb.org/t/p/original${data.backdrop_path}` : "/default_poster.jpg"}
                        fill
                        alt='Backdrop Image'
                ...
                <article className='pt-8'>
                    <div className='flex flex-col md:flex-row md:items-center gap-2 flex-nowrap sm:overflow-scroll'>
                        <h2 className='flex items-center gap-2 text-xl font-semibold text-[#404040] flex-wrap md:flex-nowrap'>
                            <span data-testid ="movie-title" className=' sm:whitespace-nowrap'>{data?.title}</span>
                            <span className='separator'/>
                            <span data-testid="movie-release-date">{dateToUTC(data?.release_date)}</span>
                            <span className='separator'/>
                            <span className='sm:whitespace-nowrap'>PG-13</span>
                            <span className='separator'/>
                            <span data-testid="movie-runtime" className='sm:whitespace-nowrap'>{data?.runtime} min</span>
                        </h2>
                    ...
                    <div className='flex flex-col lg:flex-row pt-4 md:pt-10 gap-8'>
                        <div className='text-gray-333 flex-1'>
                            ...
                            <div className='flex mt-4'>
                                <button className='text-sm text-white bg-rose-700 px-3 md:px-6 py-4 rounded-[10px]'>Top rated movie #{Math.ceil(data?.popularity)}</button>
                                <p className='border -ml-1.5 flex-1 border-[#C7C7C7] rounded-r-[10px] border-l-0 flex items-center justify-between px-4 text-xs md:text-base'>Award 9 nominations
                                    <Image
                        ...
                        <div className='flex flex-col gap-3 w-fit'>
                            ...
                            <div className='relative aspect-[1.77/1] w-full sm:w-fit rounded-[10px] flex justify-end overflow-hidden mt-4'>
                                <Image
                                    src="/image.png"
                                    fill
                                    alt='Image'
